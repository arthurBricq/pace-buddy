...
export default function RacesPage() {
  ...
  const loadRaces = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to load races'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemoveRace = async (activityId: string) => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to remove race tag'));
    }
  };
  ...
  const handleAddRace = async (activityId: string) => {
    try {
      ...
      setError(errorMessage(err, 'Failed to add race tag'));
    }
  };
  ...
  const handleRecomputeMAS = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to recompute MAS'));
    } finally {
  ...
  const handleManualOverride = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to update MAS'));
    } finally {
      setUpdatingMAS(false);
    }
  };
