...
export default function AdminDashboardPage() {
  ...
  const loadData = () => {
    setNotice(null);
    getAdminStats().then(setStats).catch((err: unknown) => setError(errorMessage(err, 'Failed to load admin stats')));
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to approve quota request'));
    }
  };
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      setError(errorMessage(err, 'Failed to reject quota request'));
    }
  };
  ...
  const handleDeleteAllData = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to delete data'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleCreateInvite = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to create invite code'));
    } finally {
