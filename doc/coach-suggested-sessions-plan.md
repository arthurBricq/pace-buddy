...
Phasing note: Phase 4 only writes `auto_matched` and `manual`. `candidate` and `rejected` are deferred to
Phase 5 — they exist to capture engine proposals the user has to confirm/dismiss, and we want the manual-link
signal from Phase 4 to calibrate the medium-confidence threshold before turning them on. The enum variants
stay in the domain code from day one (`backend/domain/src/training_session.rs`) so Phase 5 only has to wire
new code paths, not migrate data.
...
### Phase 3: Coach Suggestions [DONE]
...
### Phase 4: Auto-Match and Manual Link, with Comparison

Goal: connect executed Strava activities back to the planned sessions they completed, and produce
planned-vs-executed summaries the coach can reason about — without yet asking the user to confirm engine
guesses.

V1 scope is deliberately narrow: only two writers into `training_session_activity_matches`.

- **`AutoMatched`** when (a) the activity date falls inside the planned session's window, (b) interval-parse
  confidence is high, and (c) `session_type` is compatible. The bar is high on purpose; everything below the
  bar stays silent in v1.
- **`Manual`** when the user explicitly links an activity to a planned session from the UI.

`Candidate` and `Rejected` are intentionally not produced or surfaced in Phase 4. The reason: the medium-
confidence engine proposals they represent need a calibrated threshold and a confirm/dismiss UI, and we want
to gather labeled signal from manual links first. See Phase 5 for the reintroduction.

Tasks:

- **Matching engine.** Post-sync background task that walks newly-synced activities and writes
  `AutoMatched` rows for open `planned` sessions that clear the narrow v1 rule above. Store the score
  breakdown alongside the row even when the rule does not fire, so Phase 5 can calibrate the medium-
  confidence threshold offline.
- **Storage methods for the match table** — first real callers of `training_session_activity_matches`.
  Cover `AutoMatched` writes from the engine and `Manual` writes/deletes from the link/unlink action.
...
- **Frontend — manual link/unlink only.** On activity detail, show the matched planned session when one
  exists (auto or manual) with an Unlink action. When none exists, show a small "Link to planned session →"
  picker that lists open `planned` sessions in a bounded date window. No confirm/dismiss prompts; no
  "needs confirmation" badge on the session list. The session card grows a quiet "matched with [activity
  name]" line when a match exists. Until this phase ships, the "Mark done" button on the session card
  carries a small info badge telling the user auto-matching is not live yet.
- **Coach tools.** `list_session_matches`, `link_activity_to_planned_session`,
  ...
  + the comparison helpers. No `confirm_candidate` tool yet.
- **Coach context.** Include recently-completed sessions (with execution summaries) in the automatic
  context. No "pending confirmations" line — there are none in this phase.

Done when: a synced Strava activity that clearly completes a planned interval session gets auto-matched
without asking; the user can manually link any other activity from the activity-detail page; and the coach
can read "you ran 5 of 6 reps, second-rep pace 4 s slow" via its tools. Ambiguous cases stay silent
(unmatched) and rely on the manual fallback.

### Phase 5: Candidate Matches and Product Refinement

Goal: turn the labeled signal collected during Phase 4 into proactive engine proposals, and make the
overall loop feel like coaching rather than bookkeeping.

Tasks:

- **Reintroduce `Candidate` and `Rejected`.** Use the manual-link history from Phase 4 to calibrate a
  medium-confidence threshold: the engine writes `Candidate` rows for activities that are plausible
  matches but below the `AutoMatched` bar. Surface them in two places: a banner on Activity detail
  ("This run may complete: [session] — Confirm / Not this one") and a "needs confirmation" badge on the
  session list. Confirm flips the row to `Confirmed`; dismiss flips it to `Rejected`. `Rejected` rows are
  kept so the engine does not re-propose the same pair on a later sync.
- **"Next planned session"** surfaced on the Running Coach page so the user sees what's queued without
  leaving the chat.
- **Coach reflection on execution** after a match — natural prose follow-up the next time the user opens
  the coach.
- **Lightweight dashboard indicators** for pending confirmations (now that this phase produces them).
- **Better suggestion templates** for common goals (5K, 10K, half marathon).
- **Coach context update.** Add pending confirmations and per-session match summaries; refresh the
  `list_session_matches` tool payload accordingly.

Done when: the coach reliably uses planned and executed quality sessions to guide future workout decisions
without the user having to remind it of state, and ambiguous Strava activities surface as confirmable
candidates instead of being silently ignored.
