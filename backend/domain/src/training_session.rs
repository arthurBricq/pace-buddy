...
/// State of a link between a planned `TrainingSession` and a Strava `Activity`.
///
/// Every variant implies a referenced `activity_id`; the "no candidate" case
/// from the design doc is represented by the absence of a row, not a variant.
/// `Candidate` and `AutoMatched` both come from the matching engine — the
/// difference is confidence, not provenance.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum MatchStatus {
    /// Engine proposed this activity but confidence is below the auto-match
    /// threshold — surface it to the user for confirmation.
    Candidate,
    /// Engine proposed this activity with high enough confidence to claim the
    /// link without asking the user first.
    AutoMatched,
    /// User confirmed an engine-proposed candidate.
    Confirmed,
    /// Engine proposed this activity and the user explicitly rejected it. The
    /// row is kept so the engine does not keep re-proposing the same activity
    /// for the same session.
    Rejected,
    /// User linked this activity themselves, without an engine candidate.
    Manual,
}
