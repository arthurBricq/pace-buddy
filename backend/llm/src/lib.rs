...
pub struct ToolDefinition {
    ...
    /// If true, request strict (constrained-decoding) tool calls from the
    /// provider — the model literally cannot emit arguments that don't match
    /// `parameters`. Requires the schema to satisfy OpenAI's strict-mode
    /// rules (every property in `required`, only supported keywords, etc.).
    /// Defaults to false; tools opt in via `tool_definition_strict::<T>()`.
    #[serde(default)]
    pub strict: bool,
}
...
pub(crate) struct ChatCompletionRequestToolFunction {
    ...
    pub description: String,
    pub parameters: Value,
    /// OpenAI's `"strict": true` on the function definition triggers
    /// constrained decoding so the model can only emit schema-valid
    /// arguments. Skipped when false to keep the wire format byte-identical
    /// for tools that haven't opted in.
