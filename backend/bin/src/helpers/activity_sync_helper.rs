...
pub async fn sync_user_activities(
    ...
    let access_token = get_valid_access_token(&state.storage, &state.strava_client, user_id).await?;
    ...
    loop {
        log::info!("Fetching Strava activities page {page} for user {}", user_id);
    ...
    log::info!("Strava sync complete: {total} activities for user {}", user_id);
    Ok(total)
}
